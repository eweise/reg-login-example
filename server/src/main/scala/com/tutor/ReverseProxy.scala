...
object ReverseProxyServer extends ProxyService {
  ...
  val proxies: Map[String, URI] = config
    .getConfig("proxy")
    .entrySet().asScala
  ...
  def start(): Unit = {
    Http().bindAndHandle(appRoutes, config.getString("proxy.host"), config.getInt("proxy.port"))
  }
}