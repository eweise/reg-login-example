...
{-| This module is responsible for communicating to the Tutor API.
...
viewerChanges toMsg decoder =
    onStoreChange
        (\value ->
            let
                debug =
                    Debug.log "viewChanges" value
            in
            toMsg (decodeFromChange decoder value)
        )
...
login body decoder =
    post Endpoint.login Nothing body (decoderFromCred decoder)


register : Http.Body -> Decoder (Cred -> a) -> Http.Request a
register body decoder =
    post Endpoint.users Nothing body (decoderFromCred decoder)
